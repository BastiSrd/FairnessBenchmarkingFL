...
def load_default(url):
    """
    Loads encodes, and normalizes the Default dataset
    ...
    -------
        ...
        y (np.ndarray): Encoded target vector ('y') representing whether the client defaulted on their credit card payment (1 = default, 0 = no default).
    """
...
def load_default_age(url, sensitive_feature):
    """
    Loads the Default dataset and creates a NON-IID federated setup
...
def load_default_random(url, sensitive_feature, num_clients):
    """
    Loads Default dataset and partitions it into N randomly split clients for federated learning.

    Args:
        url (str): Path or URL to the Default dataset CSV.
        sensitive_feature (str): The column name to be used as the sensitive attribute.
